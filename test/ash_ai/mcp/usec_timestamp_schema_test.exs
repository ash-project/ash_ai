...
defmodule AshAi.Mcp.UsecTimestampSchemaTest do
  ...
  type distinct from `:utc_datetime`. Without its own schema clause it fell
  through to the generic branch, which returns a map carrying no `:type`, and
  building the tool's filter schema then raised `KeyError`.
  """
